...
app.get('/movies/:id/character/:characterId', async (req, res) => {
  ...
  const movie = await knex('movies').select('*').where({ movie_id: characterId }).first()
  const character = await knex('characters').select('*').where({ character_id: characterId }).first()
